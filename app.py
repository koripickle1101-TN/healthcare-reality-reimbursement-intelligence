...
def home_page():
    ...
    st.markdown(
        ...
        <p>
        This demo explains a healthcare operations problem: reimbursement systems may show the claim,
        authorization status, documentation status, or denial outcome, but they may not fully show the
        operational reality that created the risk.
        </p>
        </div>
        """,
        unsafe_allow_html=True
    ...
    with col1:
        st.markdown(
            ...
            <p>Where demographic, eligibility, and authorization issues often begin.</p>
            </div>
            """,
            unsafe_allow_html=True
        )
    ...
    with col2:
        st.markdown(
            ...
            <p>What staff, patients, and departments experience before the claim outcome appears.</p>
            </div>
            """,
            unsafe_allow_html=True
        )
    ...
    with col3:
        st.markdown(
            """
            <div class="metric-card">
            <h3>Revenue Cycle Risk</h3>
            <p>How upstream workflow gaps become denials, delays, rework, and patient friction.</p>
            </div>
            """,
            unsafe_allow_html=True
        )
    ...
    st.info(
        "Use the sidebar to move through the demo: System View vs Reality, Workflow Distortion Map, "
        "Simulated Case Study, Metrics Affected, Human Oversight, Executive Summary, and Disclaimer."
    )
...
def system_view_vs_reality_page():
    ...
    st.write(
        "This page compares what a reimbursement or revenue cycle system may record against what may "
        "actually happen operationally before, during, and after the patient encounter."
    )
    ...
    st.markdown(
        """
        <div class="risk-box">
        The system may capture the final claim status, but the operational cause may have started earlier
        in scheduling, eligibility verification, prior authorization, documentation preparation, or handoff communication.
        </div>
        """,
        unsafe_allow_html=True
    )
...
def workflow_distortion_map_page():
    ...
    st.write(
        "Workflow distortion happens when the system record looks stable, but the real operational process "
        "is fragmented, delayed, unclear, or dependent on manual follow-up."
    )
...
def simulated_case_study_page():
    ...
    st.markdown(
        ...
        <p>
        A patient is scheduled for a service that requires prior authorization. The appointment is completed,
        documentation is entered, and the claim is submitted. Later, the account is delayed because the
        authorization record was incomplete and documentation did not clearly support payer requirements.
        </p>
        </div>
        """,
        unsafe_allow_html=True
    ...
    st.write(
        "- Authorization ownership was unclear before the visit\n"
    ...
    st.warning(
        "The workflow looked complete inside the system, but key operational questions were unresolved before the claim moved forward."
    )
...
def metrics_affected_page():
    ...
    st.write(
        "A workflow issue may look small at the front end, but it can affect multiple healthcare operations "
        "and revenue cycle metrics downstream."
    )
...
def human_oversight_page():
    ...
    st.write(
        "This demo does not argue that systems are useless. It shows that healthcare systems still require "
        "human review because operational context may not be fully visible in structured claim, authorization, "
        "or documentation fields."
    )
    ...
    st.markdown(
        """
        <div class="risk-box">
        Human oversight is required because operational reality often lives between system fields, staff handoffs,
        payer rules, patient communication, and documentation quality.
        </div>
        """,
        unsafe_allow_html=True
    )
...
def executive_summary_page():
    ...
    st.success(
        "Recruiter explanation: This project shows that I understand how healthcare workflow issues can begin before the claim, before the denial, and before the dashboard shows a problem."
    )
